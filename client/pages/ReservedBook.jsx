...
const ReservedBook = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/reserve-book')
      .then(response => {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {reservedBooks.map((book) => (
            <div key={book._id} className="border border-gray-300 p-4 rounded-lg shadow-md bg-gray-100">
              <h3 className="text-lg font-semibold mb-2">{book.id_book.book_title}</h3>
              <p className="text-gray-700 mb-1">Author: {book.id_book.author}</p>
              <p className="text-gray-700 mb-1">Borrower: {book.id_borrower.full_name}</p>
